# -*- coding: utf-8 -*-
"""
Created on Thu Sep 28 11:11:34 2023
...
from dataloader.dataloader import Dataset_MTS

from utils.tools import EarlyStopping, adjust_learning_rate
from utils.metrics import metric
from main_basic import Exp_Basic
...
class Transformer_TimeSeries(Exp_Basic):
    def __init__(self, args):
        super(Transformer_TimeSeries, self).__init__(args)
    
    def _build_model(self):        
        model = Transformer(self.args.d_input, self.args.d_model, self.args.d_output, self.args.q_dimension, self.args.v_dimension, self.args.n_heads,
                            self.args.num_layers).float().to(self.args.gpu)
    ...
    def _get_data(self, flag):
        ...
        data_set = Dataset_MTS(root_path=args.root_path,data_path=args.data_path,flag=flag,size=[args.in_len, args.out_len],data_split = args.data_split,
                               scale=True)
    ...
    def eval(self, setting, save_pred = True, inverse = False):
        ...
        with torch.no_grad():
            for i, (batch_x,batch_y) in enumerate(data_loader):
                ...
                if (save_pred):
                    preds.append(np.mean(pred.detach().cpu().numpy(),axis = 1).T[0])
                    trues.append(true.detach().cpu().numpy().T[0][0])
        ...
        if save_pred:
            ...
            plt.plot(trues)
            plt.plot(preds)
            print('r2_Score:',r2_score(trues,preds))
